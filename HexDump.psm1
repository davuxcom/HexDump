...
# Updated by David Amenta (dave@daveamenta.com)
# ChangeLog (DA):
# - Changed to a module
# - Added input array support
...
function Format-Hex {
	...
	} elseif (!$InputArray) { write-error "No Path or byte array provided." }

	# The output will be in 16-byte lines.
	$bytesPerLine  = 16
	...
	if ($Path) {
		try {
		  ...
		  while ( $stream.Position -lt $stream.Length ) {
			...
			$fnDumpHex.Invoke($bytesRead, $buffer)
			
			write-progress -activity "Format-Hex" `
		...
		# Process the input byte array
		$fnDumpHex.Invoke($InputArray.Length, $InputArray)
	}
}
